  ...
  <form method = "post" action="" enctype="multipart/form-data">
	...
	<table align="left" width="70%">
	  ...
	  <tr>
	   ...
	   <td colspan="3">
	   <?php include('includes/state_list.php'); ?>
	   </td>
	  </tr>
	  ...
	  <tr>
	   ...
	   <td colspan="3"><input type="tel" name="contact" required placeholder="Contact" pattern="[0-9]{3}-[0-9]{3}-[0-9]{4}"/></td>
	  </tr>
...
if(isset($_POST['register'])){  
  ...
  if(!empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['confirm_password']) && !empty($_POST['name'])){
   ...
   echo "<script>alert('Sorry, your email $email address already exist in our database !')</script>";
    ...
    $run_insert = mysqli_query($con,"insert into users (ip_address,name,email,password,state,city,contact,user_address,image) values ('$ip','$name','$email','$hash_password','$state','$city','$contact','$address','$image') ");
